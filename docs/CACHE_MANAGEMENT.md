...
DataEval Workflows runs CI on GitLab and mirrors to GitHub. The docs jobs live in the
...
Pushing requires the group-level **`DATAEVAL_BUILD_PAT`** CI/CD variable (a project/group
access token with `write_repository` scope, used as the `dataeval-bot` account).
