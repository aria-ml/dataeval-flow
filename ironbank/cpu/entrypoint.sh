...
# torch's compiled extensions carry RPATH pointing at their sibling nvidia/*
# pip packages and self-resolve, but onnxruntime-gpu does not: it relies on
# the dynamic linker's search path, so libcublasLt.so etc. under
# site-packages/nvidia/*/lib go unfound and it silently falls back to
# CPUExecutionProvider unless LD_LIBRARY_PATH points at them explicitly.
if [[ "$CONTAINER_MODE" != "cpu" ]]; then
    NVIDIA_LIB_DIRS=$(python -c "
import glob, os, sysconfig
base = sysconfig.get_paths()['purelib']
print(':'.join(sorted(glob.glob(os.path.join(base, 'nvidia', '*', 'lib')))))
" 2>/dev/null)
