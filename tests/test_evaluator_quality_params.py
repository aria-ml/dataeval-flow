...
class TestDuplicatesConfig:
    def test_unset_fields_are_left_to_dataeval(self):
        assert DuplicatesConfig().constructor_kwargs() == {}
        assert DuplicatesConfig().call_kwargs() == {}

    def test_flags_map_to_image_stats(self):
        params = DuplicatesConfig(flags=["hash_basic", "hash_d4"])
    ...
    def test_unset_flags_request_what_dataeval_would_use(self):
        assert DuplicatesConfig().stats_flags() == Duplicates.Config().flags

    def test_stats_are_requested_as_the_duplicate_consumer(self):
        assert DuplicatesConfig().stats_request()["duplicate_flags"] == Duplicates.Config().flags

    def test_cluster_sensitivity_switches_on_clusters(self):
        assert DuplicatesConfig().wanted_kinds() == {InputKind.STATS}
        assert DuplicatesConfig(cluster_sensitivity=1.0).wanted_kinds() == {InputKind.STATS, InputKind.CLUSTERS}

    def test_cluster_mode_reads_one_source(self):
        params = DuplicatesConfig(cluster_sensitivity=1.0)
        assert params.check_inputs(1) is None
        problem = params.check_inputs(2)
        assert problem is not None
        assert "exactly one source in cluster mode" in problem

    def test_hash_mode_reads_any_number_of_sources(self):
        assert DuplicatesConfig().check_inputs(3) is None

    def test_call_kwargs_carry_only_set_values(self):
        assert DuplicatesConfig(per_target=True).call_kwargs() == {"per_target": True}

    def test_empty_flags_are_refused(self):
        with pytest.raises(ValidationError):
            DuplicatesConfig(flags=[])

    def test_hash_radius_reaches_dataeval(self):
        params = DuplicatesConfig(hash_radius=5)
        assert params.constructor_kwargs()["hash_radius"] == 5

    def test_unset_hash_radius_is_left_to_dataeval(self):
        assert "hash_radius" not in DuplicatesConfig().constructor_kwargs()

    def test_a_negative_hash_radius_is_refused_at_load(self):
        with pytest.raises(ValidationError):
            DuplicatesConfig(hash_radius=-1)
...
class TestOutliersConfig:
    ...
    def test_dataeval_threshold_spellings_load(self, threshold: object):
        params = OutliersConfig.model_validate({"outlier_threshold": threshold})
    ...
    def test_dataeval_refuses_an_unknown_method_at_load(self):
        with pytest.raises(ValidationError, match="DataEval rejected these parameters"):
            OutliersConfig(outlier_threshold="bogus")

    def test_unset_flags_request_what_dataeval_would_use(self):
        assert OutliersConfig().stats_flags() == Outliers.Config().flags

    def test_flags_map_to_image_stats(self):
        assert OutliersConfig(flags=["pixel"]).stats_flags() == ImageStats.PIXEL

    def test_stats_are_requested_as_the_outlier_consumer(self):
        assert set(OutliersConfig().stats_request()) == {"outlier_flags"}

    def test_cluster_threshold_switches_on_clusters(self):
        assert InputKind.CLUSTERS in OutliersConfig(cluster_threshold=2.0).wanted_kinds()
...
class TestPipelineConfigEvaluators:
    def test_entries_parse_by_type(self):
        ...
        assert [type(e) for e in config.evaluators] == [DuplicatesConfig, OutliersConfig]
