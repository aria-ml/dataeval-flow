...
from dataeval_flow._policy import policy_for, resolve_policy
...
from dataeval_flow.workflows.ood_detection._report import build_findings
...
class OODDetectionWorkflow(Workflow[OODDetectionConfig, OODDetectionResult]):
    ...
    def run(self, config: OODDetectionConfig, context: WorkflowContext) -> OODDetectionResult:
        """Score each test source against the reference with every configured OOD detector."""
        dc_items = list(context.dataset_contexts.items())

        # --- 2. Prepare datasets ---
        ref_dc, ref_dataset, test_datasets = self._prepare_datasets(dc_items)
        value_range = effective_value_range(ref_dc, config)
        ...
        detector_results, detector_names, detector_errors, is_ood_arrays = _run_all_ood_detectors(
            config, ref_embeddings, test_embeddings
        )
        ...
        if config.metadata_insights and ood_indices:
            factor_devs, factor_preds, metadata_binning = _compute_metadata_insights(
                ref_dc,
                ref_dataset,
                test_datasets,
                ood_indices,
                config.max_ood_insights,
                config,
                policy_for(context, config),
                value_range,
                context=context,
            )
    ...
    def _build_workflow_result(
        ...
        return OODDetectionResult(
            type=self.name,
            success=True,
            output=OODDetectionOutput(raw=raw, report=report),
