...
from dataeval_flow._cache import active_cache, get_or_compute_embeddings, get_or_compute_metadata
from dataeval_flow._cache import selection_repr as _sel_repr
from dataeval_flow._policy import ResolvedPolicy, policy_for
from dataeval_flow.config._schemas._task import FactorSource
...
def _label_space_digest_of(raw: DataCoverageRawOutput) -> str | None:
...
class DataCoverageWorkflow(Workflow[DataCoverageConfig, DataCoverageResult]):
    ...
    def run(self, config: DataCoverageConfig, context: WorkflowContext) -> DataCoverageResult:
        """Assess the dataset's coverage, completeness, label and metadata distributions, and gaps."""
        ...
        coverage_result, completeness_result, coverage_skipped_reason = _run_embedding_analysis(
            dc, dataset, metadata, sel_key, config
        )
        ...
        metadata_dist = _assess_metadata_distribution(metadata, config, policy)

        # ── Phase 5: Metadata gap analysis ──────────────────────────
        metadata_gaps = None
        if config.run_gap_analysis and metadata.factor_names:
        ...
        return DataCoverageResult(
            type=self.name,
            success=True,
            output=DataCoverageOutput(raw=raw, report=report),
