...
class DataCoverageRawOutput(WorkflowRawOutput):
...
class DataCoverageReport(WorkflowReport):
    """Human-readable report for data coverage workflow."""


class DataCoverageOutput(WorkflowOutput[DataCoverageRawOutput, DataCoverageReport]):
...
class DataCoverageResult(WorkflowResult[DataCoverageMetadata, DataCoverageOutput]):
    """The result of a ``data-coverage`` run.
    ...
    ------
    output.raw.coverage
        Embedding coverage results (None if no extractor)
    output.raw.coverage_skipped_reason
        Why embedding coverage was skipped despite an extractor being configured
    output.raw.completeness
        Dimensional completeness results (None if no extractor)
    output.raw.metadata_distribution
        Metadata factor distribution analysis
    output.raw.metadata_gaps
        Metadata coverage gaps by class (None if gap analysis disabled)
    output.raw.label_distribution
        Label distribution and completeness
    output.raw.ontology
