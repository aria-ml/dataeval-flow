"""The ``drift-monitoring`` workflow's config: its detectors, chunking and health thresholds."""
...
class DriftDetectorUnivariate(BaseModel):
    ...
    method: Literal["univariate"] = Field(
        default="univariate", description="Selects this drift detector: `univariate`."
    )
...
class DriftDetectorMMD(BaseModel):
    ...
    method: Literal["mmd"] = Field(default="mmd", description="Selects this drift detector: `mmd`.")
    p_val: float = Field(
        default=0.05,
        gt=0.0,
        lt=1.0,
...
class DriftDetectorDomainClassifier(BaseModel):
    ...
    method: Literal["domain_classifier"] = Field(
        default="domain_classifier", description="Selects this drift detector: `domain_classifier`."
    )
    n_folds: int = Field(
        default=5,
        ge=2,
...
class DriftDetectorKNeighbors(BaseModel):
    ...
    method: Literal["kneighbors"] = Field(
        default="kneighbors", description="Selects this drift detector: `kneighbors`."
    )
    k: int = Field(
        default=10,
        gt=0,
...
class DriftMonitoringHealthThresholds(BaseModel):
...
class DriftMonitoringConfig(WorkflowConfig[DriftMonitoringResult]):
    """The settings of one ``drift-monitoring`` entry: the detectors run against the reference, and when they warn.
    ...
    health_thresholds: DriftMonitoringHealthThresholds = Field(
        default_factory=DriftMonitoringHealthThresholds,
