...
from dataeval_flow._cache import active_cache, get_or_compute_embeddings, selection_repr
...
from dataeval_flow.workflows.drift_monitoring._report import build_findings
...
class DriftMonitoringWorkflow(Workflow[DriftMonitoringConfig, DriftMonitoringResult]):
    ...
    def run(self, config: DriftMonitoringConfig, context: WorkflowContext) -> DriftMonitoringResult:
        """Compare each test source against the reference with every configured drift detector."""
        ...
        ref_embeddings, test_embeddings, ref_labels, test_label_parts = self._extract_all_embeddings(
            ref_dc, ref_dataset, test_datasets, config
        )

        # --- 4. Run detectors ---
        detector_results, detector_names, detector_errors = _run_all_detectors(config, ref_embeddings, test_embeddings)
    ...
    def _extract_all_embeddings(
        ...
        for t_name, t_dc, t_ds in test_datasets:
            ...
            if _any_classwise(config.detectors):
        ...
        ref_labels = _extract_labels(ref_dataset) if _any_classwise(config.detectors) else None
    ...
    def _build_workflow_result(
        ...
        return DriftMonitoringResult(
            type=self.name,
            success=True,
            output=DriftMonitoringOutput(raw=raw, report=report),
