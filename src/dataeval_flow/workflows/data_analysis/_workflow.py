...
if TYPE_CHECKING:
    ...
    from dataeval_flow.config._schemas import FactorSource
...
def _resolve_outlier_flags(params: DataAnalysisConfig) -> ImageStats:
...
def _compute_split_data(
    ...
    *,
    ...
    stats_policy = stats_policy_for(
        ...
        # `DataAnalysisConfig` has no `duplicate_flags` field — duplicate detection is
...
class DataAnalysisWorkflow(Workflow[DataAnalysisConfig, DataAnalysisResult]):
    ...
    **Single omnibus** — single dataset via ``context.dataset`` (e.g. COCO/YOLO):
    ...
    def run(self, config: DataAnalysisConfig, context: WorkflowContext) -> DataAnalysisResult:
        """Analyze every split, then compare each pair of splits."""
        ...
        run_policy = policy_for(context, config)
        ...
        for split_idx, split_name in enumerate(split_order, 1):
            ...
            with contextlib.ExitStack() as stack:
                ...
                split_data[split_name] = _compute_split_data(
                    ...
                    policy=split_policy,
                    value_range=effective_value_range(dc, config),
                    context=context,
                )
        ...
        for split_name, data in split_data.items():
            ...
            sr = SplitResult(
                ...
                image_quality=_assess_image_quality(
                    data, data.outlier_flags, config.outlier_method, config.outlier_threshold
                ),
                redundancy=_assess_redundancy(data),
                label_health=_assess_label_health(data),
                bias=_assess_bias(data, config.balance, config.diversity_method, run_policy.factor_source),
            )
            split_results[split_name] = sr
            total_samples += sr.num_samples

        # ── Phase 3: Cross-split assessments ────────────────────────
        cross_split = _assess_cross_splits(split_data, config)
        ...
        return DataAnalysisResult(
            type=self.name,
            success=True,
            output=DataAnalysisOutput(raw=raw, report=report),
