...
Pure analysis over the record :func:`~dataeval_flow._binning.describe_binning` produces.
...
def find_issues(
    ...
    *,
    ...
    ----------
    record : Mapping[str, Any]
        A :func:`~dataeval_flow._binning.describe_binning` record, live or off an archived
        envelope.
...
def _name_recurring_alternative(finding: TriageFinding) -> None:
...
def _unreadable(record: Mapping[str, Any]) -> Iterator[TriageFinding]:
...
def _unbound(record: Mapping[str, Any]) -> Iterator[TriageFinding]:
...
def _degenerate(record: Mapping[str, Any], min_missing_fraction: float) -> Iterator[TriageFinding]:
...
def _identifier_finding(name: str, info: Mapping[str, Any]) -> TriageFinding | None:
...
def _floor_mass(record: Mapping[str, Any]) -> Iterator[TriageFinding]:
...
def _is_number(text: str) -> bool:
    ...
    Requiring the reading to be well-formed tells the two apart; nobody writes a measurement
    with a trailing point.
    """
    if text.endswith("."):
        return False
    try:
...
def suggest(finding: TriageFinding, *, default_bins: int = 10) -> Suggestion | None:
...
def _bin_suggestion(finding: TriageFinding, default_bins: int) -> Suggestion | None:
...
def _correction_for(finding: TriageFinding) -> Suggestion | None:
...
def to_policy_stanza(findings: Sequence[TriageFinding]) -> dict[str, Any]:
    ...
    Parameters
    ----------
    findings : Sequence[TriageFinding]
    ...
    dict[str, Any]
        ...
        :class:`~dataeval_flow.config.MetadataPolicyConfig` once ``name`` is added.
...
def incomplete_factors(findings: Sequence[TriageFinding]) -> set[str]:
    """Factor names whose suggestions are incomplete and require user input.

    Their corrections carry placeholders (e.g., ``to: null``) the user must fill in
    before the correction can be applied.
