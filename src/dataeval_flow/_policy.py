...
the bias statistics then read.  It is a decision: a bin edge is a claim about the world,
so it is defined once under the config's ``metadata:`` key and referenced by the
workflows that share it.

This module turns that configuration into something a workflow can use, and refuses the
combinations that would otherwise fail halfway through a run or silently produce
...
if TYPE_CHECKING:
    ...
    from dataeval_flow.config._schemas import AutoBinMethod, FactorSource, MetadataPolicyConfig
...
class ResolvedPolicy:
    ...
    Kept beside :attr:`encoding` and keyed with it: a roll-up or a cut decides how values
    are *grouped*; a repair decides what they **are**,
    ...
    objects DataEval hands back, not as a file: the point is to reuse a cut that was
    resolved from data.  Excluded from the
    ...
    Authored on the dataset — it describes the data instead of deciding
    anything about it — carried on the policy because it changes the injected values
...
def _correction_to_json(correction: Any) -> dict[str, Any]:
    ...
    differently would rebuild the second needlessly.
...
def _read_descriptor(path: Path, source: str) -> tuple[Mapping[str, Any], tuple[Mapping[str, Any], ...]]:
    ...
    these contents — so naming the kinds here would be a second copy of a DataEval-owned
    list.  What is checked is what a file shows to be wrong without knowing that
    vocabulary; checking it here makes it a config error, not a failure after the dataset
    walk.
    """
    if not path.exists():
        raise ValueError(
...
def _check_strict_is_earned(
    ...
    *,
    ...
    ``strict`` does not consult provenance: it applies to any recorded vocabulary,
    including one DataEval derived from a draw that nobody reviewed.  So the rule
    *a configured descriptor means a closed taxonomy* is refused: it would enforce a
    vocabulary nobody decided on and fail the run on the first new category, with an error
    calling it a "declared vocabulary" when nothing was declared.
...
def resolve_policy(
    ...
    if intrinsic_factors:
        from dataeval_flow._metadata import resolve_families, stat_names_for
...
def derive_from(policy: ResolvedPolicy, metadata: Any, descriptor: Mapping[str, Any] | None) -> ResolvedPolicy:
    ...
    draw — and their per-factor statistics sit side by side under different alphabets
    that a reader cannot know not to compare.
