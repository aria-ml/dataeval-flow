"""Parameters for the ``quality`` evaluators: DataEval's Duplicates and Outliers.
...
from dataeval_flow.config.schemas._stats import StatsPolicyRef
...
class _QualityParameters(EvaluatorParametersBase, StatsPolicyRef):
...
class DuplicatesParameters(_QualityParameters):
    """Parameters for ``quality.duplicates``, DataEval's Duplicates.
...
class OutliersParameters(_QualityParameters):
    """Parameters for ``quality.outliers``, DataEval's Outliers.
    ...
    def stats_flags(self) -> "ImageStats":
        ...
        from dataeval_flow.stats import OUTLIER_FLAG_MAP
