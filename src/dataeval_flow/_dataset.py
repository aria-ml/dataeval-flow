...
Loads datasets in HuggingFace, image folder, COCO, YOLO, and torchvision formats,
converting them to MAITE-compatible objects for evaluation.
"""
...
def load_dataset(
    ...
    *,
    ...
    >>> ds = load_dataset(Path("/data/cifar10"))  # doctest: +SKIP
    """
...
def _reject_empty_dataset(dataset: Any, root: Path, dataset_format: str, split: str | None = None) -> None:
    ...
    root exists but nothing matches the expected layout.  Fail here with the
    layout hint, before the failure surfaces as an opaque array error inside
    an evaluator.
...
def load_dataset_demo(
    ...
    :mod:`dataeval_flow.config._schemas._dataset`, never by importing what the config
    ...
    from dataeval_flow.config._schemas._dataset import _DEMO_DATASETS
...
def resolve_dataset(config: BaseModel, data_dir: Path | None = None) -> ResolvedDataset:
    ...
    """
    from dataeval_flow._cache import dataset_fingerprint
