...
source's own files where those are needed. Scores are not carried: an export writes ground
...
# Every other operation, including every `dataeval` operation not listed, transforms
# imagery. Keep this a deny-by-default list: leaving a pixel-safe operation off it makes an
# export slower (it encodes pixels it could have referenced); adding an unsafe one makes an
# export wrong (it writes a file the run never analyzed). Add a name only once confirmed
# the operation cannot touch pixels.
...
def _samples_by_datum_id(resolved: "ResolvedSource") -> dict[str, tuple[int, Any]]:
    ...
    Keyed by id, not position: a merged source's own view reorders and drops datums
    after the merge. `merge_datasets` prefixes each id with its operand's position, so
    the same key is built here. Sources whose dataset is not datamaite-backed take the
    encoded-bytes path.
    """
...
def _image_reference(
    ...
    *,
    ...
    operation in the view chain provably leaves pixels alone, and the realized image
    matches the file's size and channel count. A view that transforms imagery leaves the
    file unchanged, so referencing it would emit pixels the run never analyzed: `Crop`
    pairs the original image with boxes drawn for a smaller one, `SelectChannels` pairs
    one band with a file holding three, and a color transform changes neither shape nor
    channels and cannot be measured from them. Encode wherever the operations do not
    prove the pixels survived.
    """
...
def _ontology_entry(ontology: "ResolvedOntology | None") -> "tuple[str | None, str | None]":
    ...
    """
    from dataeval_flow._label_space import ontology_digest
...
def _recorded_runs(path: "Path") -> list[Any]:
    ...
    reason logged: an unreadable sidecar must not cost the export, and history that can be
    kept must not be dropped without a record.
    """
    import json
...
def _write_one(
    ...
    Every exception is caught. What an export can raise is open-ended: a `target: 5` in a
    `Relabel` raises `TypeError`, a classification source written as object detection
    raises `AttributeError`, and each writer has failures of its own. Every failure is
    logged with the export's name and counted into the exit code, so one bad export does
    not cost the run its others.
    """
    try:
