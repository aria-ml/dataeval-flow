...
def _brief_value(finding: Finding) -> str:
...
def _render_detail_section(finding: Finding) -> list[str]:
...
def _bin_names(info: dict[str, Any], declared: int) -> dict[int, str]:
    ...
    {attr}`ParityOutput.insufficient_data` reports for the same bin.
...
# resolution :func:`dataeval_flow._binning.describe_binning` records, so the column draws
...
def _factor_table(record: dict[str, Any], layout: tuple[int, int, int, int, int] | None = None) -> list[str]:
    ...
    The previous form gave a factor with twelve bins a line each and a factor with forty a
    single line, so the factor with the most bins got the least detail. A row apiece gives
    every factor the same room, and the shape column is drawn from the *recorded* histogram
    rather than from the factor's own cut — the chart exists to judge that cut, so drawing
    it through the cut would be circular.
...
def _render_dropped(dropped: Mapping[str, Sequence[str]]) -> list[str]:
    """Columns that never became factors, grouped by reason.

    Grouping states each reason once and names the columns it took.
    """
    if not dropped:
        return []
...
def _render_review_state(record: dict[str, Any]) -> list[str]:
    """State which parts of this encoding are reviewed and which are derived.
...
def _render_split_comparability(per_split: dict[str, Any]) -> list[str]:
    ...
    reader cannot tell not to compare.

    The verdict comes from :func:`dataeval_flow._binning.divergent_factors` rather than a
    rule of its own, so what this line claims and what the envelope's ``encoding_digest``
    and the descriptor writer will do give the same answer.
    """
    if len(per_split) < 2:
        return []
